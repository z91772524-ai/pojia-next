...
把桌面那**几套**桌面端 AI 客户端"破甲"工具，合并成**一个脚本**：一套人格，三个目标，**零第三方依赖**（纯 Python 标准库）。
...
- ✅ **装完能自证** —— 在客户端新会话里单独发一句「**破甲自检**」，收到 `破甲已生效｜目标 XXX｜v7.0` 才算真载入；不用再靠"感觉好像生效了"
- 🩺 **一条命令体检** —— `--check` 只读扫磁盘态 / 明文密钥 / 旧守护任务 / 云记忆状态 / 自证就绪，**只报分类不打印密钥原文**，退出码可直接用于脚本判断
- ☁️ **WorkBuddy 走云记忆通道**（v7.0 新增）—— 不只改它的提示词模板，还把人格写进**账号级云记忆 `memoryBlock`**（每轮自动进系统提示词），配 `Version 999999` + 只读锁防回写。**模板被升级/守护冲掉也不会失效**
...
### ☁️ WorkBuddy 云记忆通道（v7.0 新增，重点）

改 `templates\*.tpl` / `product.json` 是"改程序自带的提示词"——任何升级、自检、旧守护任务都能把它冲掉（本项目实测：某个旧守护每 30 分钟覆盖一次）。所以 v7.0 加了一条**更硬**的通道：
...
- **护照驱动还原（v7.0）**：装完会在 `managed-prompts/pojia-yijiantong/passport.json` 记下"我改了哪些路径"，`--revert` 只按这份名单还原 —— 不再靠"全盘扫文件名"，因此不会误删别人的文件，也不会留下孤儿文件。
- **不静默接管（v7.0）**：Codex 那边如果发现指令文件不是本工具写的（没有本工具身份标记），**默认拒绝接管**；确认要接管再加 `--claim`，接管前会先存快照、写一份 `*.claimed-by-pojia.txt` 说明原委。
...
**Q：怎么确认真的生效了？** —— 在客户端新会话里单独发「**破甲自检**」，应回 `破甲已生效｜目标 XXX｜v7.0`。没回就是没载入（最常见原因是没重启）。
...
| `使用说明.md` | 完整说明书 |
| `修复报告.md` | 相比原工具修掉的 18 处缺陷（B1–B18）+ 3 处合并层问题（C1–C3）+ v7.0 新增的 4 项（N1–N4），逐条对照 |
